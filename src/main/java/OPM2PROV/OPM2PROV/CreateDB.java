...
public class CreateDB {
	...
	private static Index<Node> escIdIndex;
	private Index<Relationship> reindex;
	Transaction tx;
	...
	public static void main(final String[] args) {
		if (args.length != 2) {
			...
			System.out.println("param1 database address,param2 new dataabse");
	...
	private void createDb(String path) {
		...
		// END SNIPPET: startDb
		tx = graphDb.beginTx();
		nodeIndex = graphDb.index().forNodes("esc-index");
		try {
	...
	private static Node createLibraryNode(Library library) {
		...
		Label myLabel = DynamicLabel.label("Entity");
		node23.addLabel(myLabel);
		node23.setProperty("__identifier",
	...
	private static Node createTransientDataNode(TransientData transientdata) {
		...
		Label myLabel = DynamicLabel.label("Entity");
		node23.addLabel(myLabel);
		node23.setProperty("__identifier", transientdata.getId() + "-"
		...
		nodeIndex.add(node23, "escid", transientdata.getId());
	...
	private static Node createServiceRunNode(ServiceRun servicerun) {
		...
		Label myLabel = DynamicLabel.label("Activity");
		node23.addLabel(myLabel);
		node23.setProperty("__identifier", servicerun.getInvocationId() + "-"
		...
		nodeIndex.add(node23, "escid", servicerun.getInvocationId() + "_"
				+ servicerun.getBlockUuid());
		try {
	...
	private static Node createWorkflowVersionNode(WorkflowVersion workflowrun) {
		...
		Label myLabel = DynamicLabel.label("Entity");
		node23.addLabel(myLabel);
		node23.setProperty("__identifier", workflowrun.getVersionId() + "-"
				+ workflowrun.getVersionNum());
		node23.setProperty("prov:type", "WorkflowVersion");
		node23.setProperty("escid", workflowrun.getEscId());
		nodeIndex.add(node23, "escid", workflowrun.getEscId());
		node23.setProperty("name", workflowrun.getName());
				...
				if (relationship.getType().name()
						...
						|| relationship
								...
								.equals(ProvenanceRelationshipTypes.RUN_OF
										.name())) {
					Label myLabel = DynamicLabel.label("__Hyperedge");
					...
					Relationship rs = newnode.createRelationshipTo(endnode,
						...
						.equals(ProvenanceRelationshipTypes.VERSION_OF.name())) {
					Node newnode = getNode(node);
					Node endnode = getNode(relationship.getEndNode());
					Relationship rs = newnode.createRelationshipTo(endnode,
							RelTypes.WAS_INFORMED_BY);
